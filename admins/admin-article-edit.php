...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Get the article ID from the hidden input
    $article_id = trim($_POST["id"]);

    // Check input errors before updating the database
    if (empty($title_err) && empty($category_err) && empty($content_err)) {
        // Prepare an update statement
        $sql = "UPDATE news_articles SET title = ?, category_id = ?, author = ?, content = ?, status = ? WHERE id = ?";

        if ($stmt = mysqli_prepare($conn, $sql)) {
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "issisi", $param_title, $param_category_id, $param_author, $param_content, $param_status, $param_id);
...
if (isset($_GET["id"]) && is_numeric($_GET["id"])) {
    // Check if article ID is provided and is a number (for displaying the form)
    $article_id = (int)$_GET["id"];

    // Prepare a select statement to fetch the article
    $sql = "SELECT id, title, category_id, author, content, status FROM news_articles WHERE id = ?";
    ...
    if ($stmt = mysqli_prepare($conn, $sql)) {
        ...
        if (mysqli_stmt_execute($stmt)) {
            ...
            if (mysqli_num_rows($result) == 1) {
                ...
                // Set article data for displaying in the form
                $title = $article["title"];
                ...
                // Article not found, redirect to articles page with an error
                header("location: admin-articles.php?error=article_not_found");
                exit;
            }
        } else {
            echo "Oops! Something went wrong. Please try again later.";
    ...
    // If ID is not provided or invalid, redirect to articles page
    header("location: admin-articles.php");
    exit;
}
...
if ($categories_result = mysqli_query($conn, $categories_sql)) {
    ...
    mysqli_free_result($categories_result); // Free the result set
} else {
    // Handle the error appropriately (don't just leave it silent)
    echo "Error fetching categories: " . mysqli_error($conn);
    $categories = []; // Initialize to an empty array to avoid potential errors later
}
